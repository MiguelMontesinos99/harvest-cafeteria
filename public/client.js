...
function toast(message) { const note = $('toast'); note.textContent = `✓ ${message}`; note.hidden = false; note.classList.remove('toast-visible'); requestAnimationFrame(() => note.classList.add('toast-visible')); clearTimeout(toast.timer); toast.timer = setTimeout(() => { note.classList.remove('toast-visible'); setTimeout(() => { note.hidden = true; }, 220); }, 3200); }
function addToCart(product, options = null) {
  ...
  paintCart(); $('cart-count').classList.remove('count-pop'); requestAnimationFrame(() => $('cart-count').classList.add('count-pop')); toast(`${product.nombre} se añadió al carrito`);
}
...
$('cart-open').onclick = () => { $('cart').hidden = false; requestAnimationFrame(() => $('cart').classList.add('cart-visible')); }; $('cart-close').onclick = () => { $('cart').classList.remove('cart-visible'); setTimeout(() => { $('cart').hidden = true; }, 260); };
...
paint(); paintCart();
const cartPanel = $('cart'), cartHead = cartPanel.querySelector('.cart-head'), checkoutButton = $('send-order'), cartHint = cartPanel.querySelector(':scope > small');
...
const sectionObserver = new IntersectionObserver(entries => entries.forEach(entry => { if (entry.isIntersecting) navLinks.forEach(link => link.classList.toggle('is-active', link.getAttribute('href') === `#${entry.target.id}`)); }), { rootMargin: '-35% 0px -55%', threshold: 0 });
